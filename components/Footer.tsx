    ...
    <footer className="w-full bg-surface-container-low border-t border-outline-variant/30 pt-16 pb-24 md:pb-12 text-xs sm:text-sm text-secondary">
      <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
        ...
        <div className="space-y-4">
          ...
          <p className="text-secondary text-xs leading-relaxed max-w-xs">
            Kochi’s leading premium contractor for homeowners who prioritize safety, absolute cleanliness, KSEB licensing, and professional workmanship.
          </p>
          <div className="flex gap-4 pt-2">
            <a href={`tel:${CONTACT_INFO.phone.replace(/\s+/g, '')}`} className="text-secondary hover:text-primary transition-colors">
              <span className="material-symbols-outlined text-lg">call</span>
            </a>
            <a href={CONTACT_INFO.whatsapp} target="_blank" rel="noopener noreferrer" className="text-secondary hover:text-primary transition-colors">
              {/* WhatsApp icon */}
        ...
        <div className="space-y-4">
          ...
          <p className="text-secondary text-xs leading-relaxed max-w-xs">
            Available 24/7 for major power failure diagnostics or burst pipe dispatches across Ernakulam.
          </p>
          <a
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 border-t border-outline-variant/20 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
        <p className="text-[11px] text-secondary">
          © {new Date().getFullYear()} Maria Electro Tech. All rights reserved. Registered under Kerala State Electrical Vetting Guidelines.
        </p>
